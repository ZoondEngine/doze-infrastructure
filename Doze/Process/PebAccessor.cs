...
namespace Doze.Process
{
    internal sealed class PebAccessor
    {
        ...
        internal PebAccessor(System.Diagnostics.Process process)
        {
        ...
        private ImmutableDictionary<string, string> ReadApiSetMappings()
        {
            ...
            for (var namespaceEntryIndex = 0; namespaceEntryIndex < apiSetNamespace.Count; namespaceEntryIndex++)
            {
                ...
                var namespaceEntryName = $"{Encoding.Unicode.GetString(namespaceEntryNameBytes.Span.ToArray())}.dll";
                ...
                var valueEntryName = Encoding.Unicode.GetString(valueEntryNameBytes.Span.ToArray());
