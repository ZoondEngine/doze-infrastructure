...
{
    public class ManualMapperObject : DozeObject
    {
        public ManualMapperObject()
             : base()
        { }

        public ManualMapperObject(string tag)
            : base()
        { }
        ...
        /// </summary>
        public void MapLibrary(IntPtr shadowReservedPointer)
        {
            ...
            CallInitialisationRoutines(DllReason.ProcessAttach, shadowReservedPointer);
        }

        /// <summary>
        /// Unmaps the DLL from the process
        /// </summary>
        public void UnmapLibrary(IntPtr shadowReservedPointer)
        {
            CallInitialisationRoutines(DllReason.ProcessDetach, shadowReservedPointer);
        ...
        private static string RetrieveNtdllFilePath(System.Diagnostics.Process process)
        {
        ...
        private void CallInitialisationRoutines(DllReason reason, IntPtr reservedShadowPointer)
        {
            // Call the entry point of any TLS callbacks

            if (_peImage.TlsDirectory.Value.TlsCallbackOffsets.Any(tlsCallbackOffset => !_processManager.CallRoutine<bool>(CallingConvention.StdCall, DllBaseAddress + tlsCallbackOffset, DllBaseAddress.ToInt64(), (long)reason, reservedShadowPointer.ToInt64())))
            {
            ...
            if (_peImage.PeHeaders.PEHeader.AddressOfEntryPoint != 0 && !_processManager.CallRoutine<bool>(CallingConvention.StdCall, DllBaseAddress + _peImage.PeHeaders.PEHeader.AddressOfEntryPoint, DllBaseAddress.ToInt64(), (long)reason, reservedShadowPointer.ToInt64()))
            {
        ...
        private void InitialiseSecurityCookie()
        {
            ...
            {
                securityCookieBytes[securityCookieBytes.Length - 1] += 1;
            }
